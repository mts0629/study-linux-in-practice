...
int main(int argc, char *argv[])
{
    ...
    if (argc != 6) {
        fprintf(stderr,
            "usage: %s <filename> <kernel's help> <r/w> <access pattern> <block size[KiB]>\n",
            progname);
        exit(EXIT_FAILURE);
    }
    ...
    char *buf;
    int e = posix_memalign((void **)&buf, sector_size, block_size); // Get an aligned memory
    if (e) {
        errno = e;
    ...
    if (fdatasync(fd) == -1) { // Sync the finish of I/O
